...
"""
    “Örnekleme, nesnelerin veya sınıfların herangi bir kalıtımsal donatma yapmaksızın oluşturulma sürecini belirler  
    Tüm sınıfların oluşturulma sürecini type sınıfı belirler; çünkü tüm sınıflar, type sınıfının örneğidir.
    Bu durum type sınıfını bir metaclass yapar metaclass,sınıfların nasıl oluşturulcağını tanımlayan özel bir sınıftır .

    Miras ise  sınıflar arası,sınıfların davranışlarını belirler bu,sınıfları kalıtımsal olarak donatır .
...
# metod çözümlemesi: python,bir metod çağrıldığında o metodu nerde arayacağını belirlemesidir burda mro zinciri kullanılır
...
# bu fonksiyonun amacı verilen nesnenin ve o nesnenin atalarının belirtilen sınıfın veya sınıfların örneği olup olmadığını kontrol etmek ve True/False döndürmektir
# bu fonksiyonun ismi tam olarak yaptığı iş ile örtüşmüyor bu nedenle Şu tanımı unutma -> isinstance fonksiyonu,belirtilen nesnenin örneğinde yani ait olduğu sınıfta ve bu
# sınıfın miras aldığı ata sınıflarda verilen sınıf veya sınıfları arar
