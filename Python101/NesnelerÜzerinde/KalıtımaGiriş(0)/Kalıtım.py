...
"""🔸 
Kalıtım (instantiation), sınıflardan nesne oluşturma işlemidir.  
...
# 3) Bu bir descriptor’dur (data_descriptor):
...
# 2) __getattribute__ metodu çalışır ve objenin kendisine bakılmaz çünkü, __class__, data descriptor dur öncelik __get__ metodundadır descriptor protokolü ugulanır
#    → obj.__class__.__dict__['__class__'] 

# 3) Bu attribute bir descriptor’dur → data_descriptor
...
# 2) type.__dict__['__class__'] aranır → bulunamaz mro zincirinde object sınıfında bulunur
#   -> type.__mro__[1].__dict__['__class__'] -> data descriptor'dur bu nedenle önceliği yüksektir sınıfın kendisine bakılmaz
#   direkt descriptor protokolü uygulanır
...
#     Özellikle dunder metodlar (__getattribute__, __call__ vb.)
