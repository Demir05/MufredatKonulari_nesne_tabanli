...
# Decorator'e parametre verme;

# Decorator'e parametre verme örneği:

# En dıştaki fonksiyon: bu, decorator'e parametre verilmesini sağlar
def repeat(n):  
    print("1) repeat() çağrıldı")  # sadece bir defa → decorator tanımında

    # Orta katman → gerçek decorator fonksiyonu
    def decorator(func):
        print("2) decorator() çağrıldı")  # @repeat satırı görüldüğünde çalışır
        
        # En içteki fonksiyon: asıl davranışı değiştiren sarıcı (wrapper)
        def wrapper(*args, **kwargs):
            print("5) wrapper() fonksiyonu çağrıldı")  # f("demir") çağrılınca çalışır
            for i in range(n):
                print(f"6.{i+1}) func('{args[0]}') çağrılıyor")
                result = func(*args, **kwargs)
            return result
        
        print("3) wrapper fonksiyonu, decorator'den return ediliyor")
        return wrapper  # → f = wrapper
    
    print("4) decorator fonksiyonu return ediliyor")
    return decorator  # → repeat(3) = decorator


@repeat(3)  # repeat(3) bir fonksiyon döndürür (decorator)
def f(name):
    print(f"7) Merhaba {name}")


"""
@repeat(3)
↓
f = repeat(3)(f) ->  repeat çağrılır → decorator döner → decorator(f) çağrılır → wrapper döner → f = wrapper
1) repeat(3) çağrılır → n=3
2) repeat(), decorator fonksiyonunu döner

@decorator(f)
↓
3) decorator(f) çağrılır → f fonksiyonu parametre olarak alınır
4) decorator, wrapper fonksiyonunu return eder
→ f = wrapper  artık f çağrıldığında wrapper çalışır

f("demir")
↓
5) wrapper() çağrılır (f aslında wrapper'dır)
6) for döngüsünde 3 defa orijinal func("demir") çağrılır
7) her seferinde: print(f"Merhaba {name}") çalışır
"""   