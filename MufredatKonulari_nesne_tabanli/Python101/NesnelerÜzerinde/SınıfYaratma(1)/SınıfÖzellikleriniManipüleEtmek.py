...
# Eğer sınıf içinde __setattr__ override edilmemişse, Python metod çözümlemesi (MRO) yapar ve object.__setattr__ metoduna ulaşarak doğrudan onu çağırır.
...
# 🔥 __setattr__() METODU – ÇAĞRI ZİNCİRİ
...
# 1) obj.__setattr__('x', 10) çağrılır (çünkü bu bir davranışsal işlemdir)
# 2) Python çözümleme için obj'nin sınıfına bakar:
#      → type(obj).__setattr__(obj, 'x', 10)
# 3) Eğer __setattr__ sınıfta override edilmemişse:
#      → object.__setattr__(obj, 'x', 10) çalışır

# 4) Eğer obj.__class__.__dict__['x'] bir descriptor nesnesiyse ve __set__ metodu varsa:
#      → descriptor.__set__(obj, 10) doğrudan çağrılır
#      → __setattr__ devreye girmez

# ⚠️ Not: self.x = y gibi atamalar da aynı zinciri takip eder
# Yani __init__ içinde bile olsalar, __setattr__ devreye girer.
...
# → A bir sınıf olduğundan, Python bunu:
#      → type(A).__setattr__(A, 'x', 42) şeklinde yürütür

# Çünkü tüm sınıflar, type sınıfının örnekleridir
# Yani sınıf düzeyinde yapılan davranışsal işlemler → type tarafından kontrol edilir

# Eğer A.__dict__['x'] bir descriptor ise ve __set__ içeriyorsa:
#      → descriptor.__set__(A, 42) doğrudan çağrılır
#      → __setattr__ çağrılmaz

# ========================================
# 📝 NOT – EL İLE YAPILAN DESCRIPTOR ÇÖZÜMLEMESİ
# ========================================

# Eğer biz bu zinciri manuel olarak çalıştırmak istersek:

# 1) __dict__ üzerinden descriptor’a erişiriz:
#    descriptor = type(obj).__dict__['__setattr__']

# 2) Ardından descriptor protokolünü uygularız:
#    bound_method = descriptor.__get__(obj, type(obj))

# 3) Ve çağırırız:
#    bound_method('x', 10)

# 💡 Bu teknik, Python’un normalde yapmadığı, senin elle başlattığın çözümlemedir.
#    Eğitimsel, inceleme veya monkey-patching gibi ileri seviye amaçlar için kullanılır.
...
#      → type(A).__delattr__(A, 'attr') çalışır
